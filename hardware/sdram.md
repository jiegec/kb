...
1. 接口复杂，访问数据前需要首先激活一个 row，再读取 row 内的数据
1. 控制器也比较复杂，需要定期刷新内存
1. 容量较大，由于 row 和 column 复用了地址线，单内存条可以做到 GB 级别的容量
...
MT40A1G8 的结构框图（图源 [Micron Datasheet](https://media-www.micron.com/-/media/client/global/documents/products/data-sheet/dram/ddr4/8gb_ddr4_sdram.pdf?rev=8634cc61670d40f69207f5f572a2bfdd)）
...
DDR4 Burst 类型和 Column 次序（图源 [JESD9-4D DDR4](https://www.jedec.org/document_search?search_api_views_fulltext=jesd79-4%20ddr4)）
...
1. 第二步，在 row 里读出想要的数据

但是每个 Bank 同时只能取出来一个 row，所以如果两次读涉及到了不同的 row，那么需要：

1. 第一步，先把第一次读的数据所在的整个 row 取出来
1. 第二步，在 row 里读出想要的数据
1. 第三步，把第一次读的 row 放回去
1. 第四步，把第二次读的数据所在的整个 row 取出来
1. 第五步，在 row 里读出想要的数据
...
1. RCD（ACT to internal read or write delay time）：从 Activate 到下一个读或写请求的时间
1. RP（RRE command period）：发送 Precharge 命令到下一个命令的时间
1. RAS（ACT to PRE command period）：从 Activate 到 Precharge 之间的时间
1. RC（ACT to ACT or REF command period）：从 Activate 到下一个 Activate 或者 Refresh 之间的时间
1. RTP（Internal READ Command to PRECHARGE command delay）：从 Read 到 Precharge 之间的时间

于是上面的流程需要的时间要求就是：

1. 第一步，Activate，取出第一个 row
1. 第二步，Read，第一步和第二步之间要隔 RCD 的时间，从 Read 发送地址到得到数据要等待 CL 的时间
1. 第三步，Precharge，第一步和第三步之间要隔 RAS 的时间，第二步和第三步之间要隔 RTP 的时间
1. 第四步，Activate，取出第二个 row，第一步和第四步之间要隔 RC 的时间，第三步和第四步之间要隔 RP 的时间
1. 第五步，Read，第四步和第五步之间要隔 RCD 的时间，从 Read 发送地址到得到数据要等待 CL 的时间

根据这个流程，可以得到以下的结论：

1. 访问带有局部性的数据性能会更好，只需要连续地进行 Read，减少 Activate 和 Precharge 次数
1. 不断访问不同的 row 的数据，会导致需要来回地 Activate，Read，Precharge 循环
1. 访问 row 和访问 row 中的数据分成两个阶段，两个阶段可以使用同样的地址信号，使得内存总容量很大
1. 而如果访问总是命中同一个 row，就不需要 Activate 和 Prechage，可以持续 Read，获得接近理论的传输速率，如图：

DDR3 同一个 Row 内的的连续读（图源 [JESD9-3F DDR3](https://www.jedec.org/sites/default/files/docs/JESD79-3F.pdf)）
...
DDR4 的非连续读（图源 [JESD9-4D DDR4](https://www.jedec.org/document_search?search_api_views_fulltext=jesd79-4%20ddr4)）
...
DDR3 SDRAM 存储部分（图源 [Micron Datasheet](https://media-www.micron.com/-/media/client/global/documents/products/data-sheet/dram/ddr3/1gb_ddr3_sdram.pdf?rev=22ebf6b7c48d45749034655015124500)）

DDR4 SDRAM 512 Meg x 16 存储部分（图源 [Micron Datasheet](https://media-www.micron.com/-/media/client/global/documents/products/data-sheet/dram/ddr4/8gb_ddr4_sdram.pdf?rev=8634cc61670d40f69207f5f572a2bfdd)）
...
DDR4 不同 Bank Group 的连续读（图源 [JESD9-4D DDR4](https://www.jedec.org/document_search?search_api_views_fulltext=jesd79-4%20ddr4)）
...
1. DDR4 引入了 Bank Group，所以多出了 BG0-BG1 引脚作为 Bank Group 地址
1. DDR4 引入了 Bank Group 后，总 Bank 数量是 (4\*4=16)；DDR3 的 Bank 数量是 8
1. DDR3 中的 RAS_n、CAS_n 和 WE_n 在 DDR4 中被复用为了 A14-A16，目的是更大的容量
1. DDR4 额外添加了 ACT_n 控制信号
...
SDRAM 的两种信号连接方式（图源 [Versal ACAP PCB Design User Guide (UG863)](https://docs.xilinx.com/r/en-US/ug863-versal-pcb-design/Signals-and-Connections-for-DDR4-Interfaces)）
...
Clam-shell Topology（图源[Versal ACAP Programmable Network on Chip and Integrated Memory Controller LogiCORE IP Product Guide (PG313)](https://docs.xilinx.com/r/en-US/pg313-network-on-chip/Clamshell-Topology) ）
...
SDRAM 引脚镜像（图源[Versal ACAP PCB Design User Guide (UG863)](https://docs.xilinx.com/r/en-US/ug863-versal-pcb-design/Utilizing-Address-Mirroring-to-Ease-Clamshell-Routing)）
...
Write Leveling 所要达到的 DQS 与 CK 同步的效果
...
HBM 的封装形式（图源 [A 1.2V 20nm 307GB/s HBM DRAM with At-Speed Wafer-Level I/O Test Scheme and Adaptive Refresh Considering Temperature Distribution](https://picture.iczhiku.com/resource/ieee/WYifSuFTZuHLFcMV.pdf)）
...
1T DRAM Cell 内部构造（图源 [Berkeley EECS 151/251A Spring 2019](https://inst.eecs.berkeley.edu/~eecs151/sp19/files/lec16-ram.pdf)）
...
DDR5 引入了 DFE 来改进信号完整性。关于 DFE，详见 [信号处理](https://jia.je/kb/hardware/signal_processing.html) 中的介绍。

DDR5 支持 4-Tap DFE，也就是说，考虑的是最近四个符号对当前符号带来的干扰，对应四个系数。为了测量出这四个系数，在训练的时候，首先发出一个脉冲，然后观察它的响应，通过不断调整系数，抵消它对后续符号的干扰。

DDR5 DFE（图源 [Meeting the 5 Key DDR5 Test Challenges as We Migrate to Next Gen Memory](https://www.tek.com.cn/-/media/china-marketing-documents/ddr5_webinar_oct2020_tektronix_bitwise.pdf)）
