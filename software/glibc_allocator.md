...
1. 返回这个空闲块的地址（实际上是被分配给用户的部分，即跳过空闲块头部）
...
1. 给 tcache 和 fast bin 添加了更多对齐检查，例如在 64 位下，块总是会对齐到 16 字节
...
性能方面，在 commit [malloc: Improve performance of \_\_libc_malloc](https://github.com/bminor/glibc/commit/b0897944cc3081e019b39981790051f7ee127406) 中对 \_\_libc_malloc 进行了拆分：

1. \_\_libc_malloc 尝试进行 tcache 分配，如果成功则直接返回
1. 如果 tcache 分配失败，再尾调用 \_\_libc_malloc2，完成原来剩下的分配内容
...
1. glibc 2.43 开始，fast bin 被删除了，tcache 每个 bin 的默认大小从 7 改成了 16
