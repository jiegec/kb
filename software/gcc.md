...
1. 构建 binutils
1. 构建一个不依赖 libc 的 gcc+libgcc，编译的时候设置：
   1. --without-headers：没有 C 头文件
   1. --disable-threads：没有 pthread
   1. --disable-shared：动态链接依赖 libc
1. 构建 libc，如果 libc 需要 linux 头文件，则还需要在 linux 源码中进行 headers_install
1. 再构建一个依赖 libc 的 gcc+libgcc
