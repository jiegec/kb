...
- Import existing RAID: auto `sudo mdadm --assemble --scan`, manual `sudo mdadm --assemble /dev/md0 /dev/sda1 /dev/sdb1 /dev/sdc1`, pass `--update=devicesize` if underlying partition size has changed
