...
1. O32 ABI: original/old 32 位 ILP32 ABI，32 位 MIPS 用的多，`mips*-linux-gnu` target
   ...
   1. `$2-$3($v0-$v1)` 传返回数据
   1. 16 个浮点寄存器
1. N64 ABI: new 64 位 LP64 ABI，64 位 MIPS 用的多，`mips*64*-linux-gnuabi64` target
   1. `$4-$11($a0-$a7)` 传整数参数
   1. `$f12-$f19` 传浮点参数
   1. `$2-$3($v0-$v1)` 传返回数据
   1. 32 个浮点寄存器
1. N32 ABI：把 N64 从 LP64 移植 ILP32，即只用 32 位地址，但寄存器还是 64 位，`mips*64*-linux-gnuabin32` target
1. O64 ABI: 把 O32 移植到 64 位，64 位寄存器
1. EABI: 有 32 位和 64 位版本：eabi32/eabi64
