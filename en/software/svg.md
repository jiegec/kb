...
1. `L 100 0`（Line To），从当前位置画一条线到 (100, 0)
...
1. MoveTo（移动）: M, m
1. LineTo（画线）: L, l（画任意方向的线）, H, h（画横向线）, V, v（画纵向线）
1. Cubic Bézier Curve（三次贝塞尔曲线）: C, c, S, s
1. Quadratic Bézier Curve（二次贝塞尔曲线）: Q, q, T, t
1. Elliptical Arc Curve（椭圆上的弧线）: A, a
1. ClosePath（回到起始点）: Z, z
