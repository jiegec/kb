...
Treiber Stack（图源 [Systems Programming: Coping With Parallelism Page 18](https://dominoweb.draco.res.ibm.com/reports/rj5118.pdf)）
...
Treiber Stack 的 ABA 问题（图源 [Systems Programming: Coping With Parallelism Figure 10 on Page 19](https://dominoweb.draco.res.ibm.com/reports/rj5118.pdf)）

为了解决这个问题，需要把链表头指针和一个整数绑在一起，二者同时 CAS：每次更新指针的时候，就把这个整数加一，这样就可以区分出前后两个 A 指针了，即使它们指针的值相同，但是整数不同，依然可以正常区分。这需要硬件的支持，通常叫做 Double-wide compare and swap，详见 [原子指令](https://jia.je/kb/en/software/atomic_instructions.html)；如果硬件不支持 Double-wide compare and swap，同时虚拟地址没有占用完整的指针长度（例如 64 位下虚拟地址通常只有 48 位），可以复用指针的高位来保存这个整数。使用 Double-wide compare and swap 的汇编版本：

Treiber Stack 修正 ABA 问题的版本（图源 [Systems Programming: Coping With Parallelism Figure 11 on Page 20](https://dominoweb.draco.res.ibm.com/reports/rj5118.pdf)）
...
1. 引用计数：对结点进行引用计数，当引用计数降为零的时候再释放
1. Hazard Pointer：见下
...
Hazard Pointers 的 Scan 函数（图源 [Simple, fast, and practical non-blocking and blocking concurrent queue algorithms Figure 3](https://dl.acm.org/doi/10.1145/248052.248106)）
...
Hazard Pointers（图源 [Simple, fast, and practical non-blocking and blocking concurrent queue algorithms Figure 4](https://dl.acm.org/doi/10.1145/248052.248106)）

有了 Hazard Pointers 机制以后，再改写 Treiber Stack 的 pop 函数，就可以实现内存回收了：

Hazard Pointers 应用到 Treiber Stack（图源 [Simple, fast, and practical non-blocking and blocking concurrent queue algorithms Figure 8](https://dl.acm.org/doi/10.1145/248052.248106)）
...
Lock Free Queue（图源 [Simple, fast, and practical non-blocking and blocking concurrent queue algorithms](https://dl.acm.org/doi/10.1145/248052.248106)）
...
Lock Free Set（图源 [High performance dynamic lock-free hash tables and list-based sets](https://dl.acm.org/doi/10.1145/564870.564881)）
