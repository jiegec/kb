...
1. 递减 `scan` 指针，直到找到一个需要被挪到前面的对象（要求 `scan > free`）
1. 把处于 `scan` 指针的对象移动到 `free` 指向的位置
1. 因为可能有其他对象拥有对 `scan` 指向的对象的指针，为了保证后续指针可以被正确地更新，在 `scan` 指针的位置，留下一个指向 `free` 的信息，表示之前在 `scan` 指针的位置这里有个对象，现在被挪到了 `free` 这个位置
1. 循环以上过程，直到所有对象都被紧密地保存，此时 `free >= scan`
...
1. 第二个步骤是 updateReferences，更新对象的字段，把各字段的指针改成对应对象的 forwardingAddress，也就是去指向新的 compact 后的地址
1. 第三个步骤是 relocate，即按照地址从低往高的顺序，把各对象挪到第一步已经计算好的位置上；地址的分配和移动顺序，保证了移动的时候不会覆盖掉不该覆盖的对象
...
1. 通过遍历，构造出一个链表：N -> C -> B -> A，也就是说把引用了 N 这个对象的字段都放到一个链表里面，这个时候就可以从 N 知道有哪些字段引用了 N
1. 那么在要移动 N 的时候，就可以在知道 N 的新地址的同时，遍历这个链表，恢复出原来的连接关系（即 A B C 三个对象分别有字段指向了 N），并修正 A B C 三个对象的字段的指针，让它们指向 N 的新地址
...
1. 第二轮，再次遍历对象，给对象分配 compact 后的地址，此时通过链表，可以找到所有引用当前对象的字段，把指针改写回正确的连接关系，并改成 compact 后的地址；这一轮过后，每个对象的字段都指向了对应对象的新地址
1. 第三轮，再次遍历对象，把对象挪到正确的位置
...
1. 第二轮，地址从低往高遍历，此时前向的引用都被修正了，可以移动对象到被 compact 后的地址，不用担心出错；而此时第一轮也把后向的引用都加入了链表当中，此时再修正后向的引用，由于地址比当前地址大的对象都没有被移动，所以修改不会出现问题
...
1. 虽然不知道这个块内有多少个对象，但是通过 bitmap，可以知道这个块内分配了多少字节的数据用于保存对象
1. 以块的粒度，按照顺序给每个块分配空间，得到每个块在 compact 后的地址

有了这个信息以后，再遍历堆里的活跃对象，此时，可以利用预处理好的信息，计算出每个对象在 compact 后的地址：

1. 首先根据对象的地址，找到它属于哪个块，并处于块内的第几个对象，它前面要保存多少字节的数据
1. 找到对应块 compact 后的地址，加上块内的偏移，就得到了这个对象 compact 后的地址
...
1. 维护两个指针：`scan` 一开始指向 tospace 空间的最开头，`free` 指向 tospace 中空闲的空间的开头
1. 从 `scan` 指针开始，扫描 tospace 已有的对象，找到它所引用的在 fromspace 中还没复制到 tospace 的对象，把 fromspace 中的对象复制到 tospace 中 `free` 指向的空闲空间，更新 forwardingAddress，然后更新 `free` 指向剩余的空闲空间
1. 当 `scan` 等于 `free` 的时候，所有的 fromspace 活跃对象都已经复制完毕
...
1. Incremental：把一次完整的垃圾回收操作拆成多次，一次长的 pause 被拆分成多个短的 pause，主要目的是减少最大的 pause time，改进响应性
1. Concurrent：不 stop the world，mutator 和 collector 可以在不同的处理器核心上同时进行
...
1. 并且从已有的灰色对象到白色对象没有路径

为了解决这个问题，在 mutator 操作对象的时候，插入额外的 read/write barrier，从而保证正确性，例如：

1. Steele write barrier：如果建立了一条从黑色对象到白色对象的边，把黑色对象变成灰色对象
1. Dijkstra et al. write barrier：如果建立了一条从黑色对象到白色对象的边，把白色对象变成灰色对象
1. Baker read barrier: 如果读取了灰色对象的字段里的白色对象，把白色对象变成灰色对象
...
1. Android Runtime GC，从 Lollipop (Android 5) 开始到 Marshamallow (Android 6): Concurrent Mark-And-Sweep + Generational
1. Android Runtime GC，从 Oreo（Android 8）开始：Concurrent Copying（`Starting with Android 8 (Oreo), the default plan is Concurrent Copying (CC).`）
1. Android Runtime GC，从 Q（Android 10）开始：Concurrent Copying + Generational（`CC extends to be a generational GC in Android 10 and higher.`）
