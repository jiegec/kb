...
1. 插入一系列的总是跳转的分支（下称 dummy branch）
1. 根据第一步的随机数，让一个分支和第一个分支以相同方向跳转或不跳转
...
1. 把一个依赖随机数的分支指令，放置到特定的位置，使得分支地址和目的地址的一些位设成 0，一些位设成 1
1. 再添加一个分支指令，也依赖第二步的随机数选择跳转或不跳转
...
1. 假如第二步跳转了以后，得到的 PHR 依然是零，那么分支预测器在预测第三步的分支的时候，无法区分第二步往跳转了还是没跳转，于是第三步的分支预测错误率就会接近 50%
...
1. 中等大小的 PHT 使用了 132 位的 PHR 历史
1. 最大的 PHT 使用了完整 388 位的 PHR 历史
...
1. perf_event_attr 的 config 的取值见 [dougallj/applecpu](https://github.com/dougallj/applecpu/blob/0e6bc3f6038fa7b3959ab66b33ae25b707edc186/timer-hacks/bench.py#L85) 的 COUNTER_NAMES，例如测量周期数就是 0x02，测量分支错误预测次数就是 0xcb
1. perf_event_attr 的 exclude_guest 必须设为 1，否则会得到 EOPNOTSUPP
...
以上结果是用 [python3 cpu_microarchitecture_apple_pmc_dump.py /usr/share/kpep/a14.plist](https://jia.je/kb/hardware/cpu_microarchitecture_apple_pmc_dump.py) 命令跑出来的。
