...
1. Mesh：以 2D Mesh 为例，把结点放在一个网格里，每个结点最多连接四个相邻结点：在内部的连接四个相邻结点，在边上的只连接三个相邻结点，在角上的只连接两个结点
1. Torus：在 Mesh 的基础上，让边和角上的结点也连接四个结点：边上的结点连到对侧最远的结点，角上的结点连到另外两个角

Ring，Mesh 和 Torus（图源 [On Chip Networks, Second Edition](https://link.springer.com/book/10.1007/978-3-031-01755-1)）
...
Fat Tree（图源 [Wikipedia](https://en.wikipedia.org/wiki/Fat_tree#/media/File:Fat_tree_network.svg)）

可以从以下几个方面评估一个拓扑的属性：

1. 结点的度：连接的结点数
1. 直径：结点间距离的最大值
1. 等分宽度：网络被切成两半（或者几乎两半）的时候，切割的最小边数
...
路由协议死锁的情况（图源 [On Chip Networks, Second Edition](https://link.springer.com/book/10.1007/978-3-031-01755-1)）
...
Mesh 的转向（(a) 是所有可能，(b) 是 X-Y 路由）（图源 [On Chip Networks, Second Edition](https://link.springer.com/book/10.1007/978-3-031-01755-1)）

这种拐法可以保证不出现死锁，但是有更加宽松的转向方式，也可以保证不出现死锁，例如下面的三种：

Mesh 的三种转向模型（图源 [On Chip Networks, Second Edition](https://link.springer.com/book/10.1007/978-3-031-01755-1)）
...
此时，由于两个结点间不止一个 Channel，即使人为地禁用一个 Channel，也可以正常传输，并且也打破了环。以下图为例：下图 (a) 左侧中 (n_1) 到 (n_4) 是组成环的四个结点，四个 Channel 互相依赖，因此 Channel Dependency Graph 出现了环（下图 (a) 右侧）；引入了 Virtual Channel 以后，相邻结点间就有了两个 Channel，此时禁用 (c\_{00}) 通道，并且约定 Channel 的优先级，那么 Channel Dependency Graph 就不再成环（下图 (b) 右侧），并且由于数据不会绕着环转两圈，不影响可达性。

使用 Virtual Channel 解决死锁问题（图源 [Deadlock-Free Message Routing in Multiprocessor Interconnection Networks](https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=1676939)）
