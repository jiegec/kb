...
1. complex interface, you need to activate a row before accessing data, and then read the data in the row
1. the controller is also complex and requires periodic memory refreshing
1. large capacity, because the row and column multiplexed address lines, a single memory module can achieve GB level capacity
...
Block diagram of MT40A1G8 (Source [Micron Datasheet](https://media-www.micron.com/-/media/client/global/documents/products/data-sheet/dram/ddr4/8gb_ddr4_sdram.pdf?rev=8634cc61670d40f69207f5f572a2bfdd))
...
DDR4 Burst Type and Column Order (Source [JESD9-4D DDR4](https://www.jedec.org/document_search?search_api_views_fulltext=jesd79-4%20ddr4))
...
1. in the second step, read the desired data in the row
...
1. in the second step, read the desired data in the row
1. step 3, put the first read row back
1. step 4, take out the whole row where the data of the second read is located
1. step 5, read the desired data in the row
...
1. RCD (ACT to internal read or write delay time): the time from Activate to the next read or write request
1. RP (RRE command period): the time between sending a Precharge command and the next command
1. RAS (ACT to PRE command period): the time between Activate and Precharge
1. RC (ACT to ACT or REF command period): the time between Activate and the next Activate or Refresh
1. RTP (Internal READ Command to PRECHARGE command delay): the time between Read and Precharge

So the above process requires the following time requirements:

1. the first step, Activate, take out the first row
1. the second step, Read, the time between the first and second steps should be separated by RCD, and the time to wait for CL from Read to send address to get data
1. step 3, Precharge, the time between step 1 and step 3 should be separated by RAS, and the time between step 2 and step 3 should be separated by RTP
1. step 4, Activate, take out the second row, the time between step 1 and step 4 should be separated by RC, and the time between step 3 and step 4 should be separated by RP
1. step 5, Read, the time between step 4 and step 5 should be separated by RCD, the time to wait for CL from Read to send address to get data
...
1. constantly accessing data from different rows will lead to a back and forth Activate, Read, Precharge cycle
1. accessing the row and accessing the data in the row are divided into two phases, and both phases can use the same address signals, making the total memory capacity large
1. if the access always hits the same row, the transfer rate can be close to the theoretical one, as shown in Figure 2

DDR3 Sequential Reads within the Same Row (Source [JESD9-3F DDR3](https://www.jedec.org/sites/default/files/docs/JESD79-3F.pdf))
...
DDR4 Nonconsecutive Reads (Source [JESD9-4D DDR4](https://www.jedec.org/document_search?search_api_views_fulltext=jesd79-4%20ddr4))
...
DDR3 SDRAM Storage Part (Source [Micron Datasheet](https://media-www.micron.com/-/media/client/global/documents/products/data-sheet/dram/ddr3/1gb_ddr3_sdram.pdf?rev=22ebf6b7c48d45749034655015124500))

DDR4 SDRAM Storage Part (Source [Micron Datasheet](https://media-www.micron.com/-/media/client/global/documents/products/data-sheet/dram/ddr4/8gb_ddr4_sdram.pdf?rev=8634cc61670d40f69207f5f572a2bfdd))
...
DDR4 Consecutive Reads from Different Banks (Source [JESD9-4D DDR4](https://www.jedec.org/document_search?search_api_views_fulltext=jesd79-4%20ddr4))
...
1. Bank: Each Bank has only one Row activated at the same time
1. Row: Activate/Precharge unit
1. Column: each Column holds n cells, n is the bit width of SDRAM
1. Cell: each cell holds 1 bit of data

In fact, there are several layers outside the SDRAM:

1. Channel: the number of channels of the processor's memory controller
1. Module: there can be multiple memory sticks connected to the same Channel
1. Rank: multiple DDR SDRAM chips are stitched together in width, one to four Ranks can be put down on a Module. these Ranks share the bus, each Rank has its own chip select signal CS_n, which is actually stitching SDRAM chips in depth
1. Chip: A DDR SDRAM chip; such as a 64-bit wide Rank, is made by using 8 x8 chips stitched together across the width
...
DDR3 and DDR4 Schematics

The differences between DDR3 and DDR4:

1. address signal: DDR3 has A0-A14, DDR4 has A0-A17, where A14-A16 are multiplexed pins
1. DDR4 introduces Bank Group, so there are extra pins BA0-BA1.
1. RAS_n, CAS_n and WE_n in DDR3 are reused as A14-A16 in DDR4
1. DDR4 adds an additional ACT_n control signal
...
Two types of signal connections for SDRAM (Source [Versal ACAP PCB Design User Guide (UG863)](https://docs.xilinx.com/r/en-US/ug863-versal-pcb-design/Signals-and-Connections-for-DDR4-Interfaces))
...
Clam-shell Topology (Source [Versal ACAP Programmable Network on Chip and Integrated Memory Controller LogiCORE IP Product Guide (PG313)](https://docs.xilinx.com/r/en-US/pg313-network-on-chip/Clamshell-Topology) )
...
SDRAM Pin Mirroring (Source[Versal ACAP PCB Design User Guide (UG863)](https://docs.xilinx.com/r/en-US/ug863-versal-pcb-design/Utilizing-Address-Mirroring-to-Ease-Clamshell-Routing))
...
DQS and CK synchronization to be achieved by Write Leveling
...
Results of DQS sampling of CK with different DQS delays
...
1. The controller enumerates the latency of DQS, reads out the DQ results for each DQS latency, and gets a 0-1 string, for example: `00111111111111111111110000`, that is, as the latency increases, it samples to 0, then to 1, and finally to 0 again
1. find a DQS delay that causes the DQ to change from 0 to 1, then the DQS will be synchronized with the CK according to this delayed output
1. Set the SDRAM to end Write Leveling mode
...
1. set the latency to loop from 0
1. at each latency setting, read out the data and compare the read results with the previously written data
1. count which delay conditions, the read data is accurate. accurate is recorded as 1, inaccurate is recorded as 0
1. find the accurate range of 1, and take the midpoint as the final calibration result
...
HBM Packaging (Source [A 1.2V 20nm 307GB/s HBM DRAM with At-Speed Wafer-Level I/O Test Scheme and Adaptive Refresh Considering Temperature Distribution](https://picture.iczhiku.com/resource/ieee/WYifSuFTZuHLFcMV.pdf))
...
1T DRAM Cell Internal Structure (Source [Berkeley EECS 151/251A Spring 2019](https://inst.eecs.berkeley.edu/~eecs151/sp19/files/lec16-ram.pdf))
...
DDR5 introduces DFE (Decision Feedback Equalization) to enhance signal integrity. For details about DFE, refer to the introduction in [Signal Processing](https://jia.je/kb/en/hardware/signal_processing.html).
...
DDR5 DFE (Source [Meeting the 5 Key DDR5 Test Challenges as We Migrate to Next Gen Memory](https://www.tek.com.cn/-/media/china-marketing-documents/ddr5_webinar_oct2020_tektronix_bitwise.pdf))
