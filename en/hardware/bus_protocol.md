...
1. 地址（`addr`）：例如 32 位地址
1. 数据（`w_data` 和 `r_data`）：分别是写数据和读数据，宽度通常为 32 位或 64 位，也就是一个时钟周期可以传输的数据量
1. 读还是写（`we`）：高表示写，低表示读
...
1. 如果 master 发起请求，同时 slave 不能接收请求，即 `valid_o=1 && ready_i=0`，此时 master 要保持 `addr_o` `we_o` `data_o` 和 `be_o` 不变，直到请求结束，这一点非常重要
...
1. `STB_O`：高表示 master 要发送请求，即简易总线协议中的 `valid_o`
1. `ACK_I`：高表示 slave 处理请求，即简易总线协议中的 `ready_i`
1. `ADR_O`：master 想要读写的地址，即简易总线协议中的 `addr_o`
1. `WE_O`：master 想要读还是写，即简易总线协议中的 `we_o`
1. `DAT_O`：master 想要写入的数据，即简易总线协议中的 `data_o`
1. `SEL_O`：master 读写的字节使能，即简易总线协议中的 `be_o`
1. `DAT_I`：master 从 slave 读取的数据，即简易总线协议中的 `data_i`
1. `CYC_O`：总线的使能信号，无对应的简易总线协议信号
...
1. W（Write）：写请求的数据会通过 W Channel 发送给 Slave
1. B：写响应会通过 B Channel 发送给 Master
1. AR（Address Read）：读请求的地址会通过 AR Channel 发送给 Slave
1. R（Read）：读响应会通过 R Channel 发送给 Slave
...
1. 通过 W Channel 发送要写入的数据，支持 Burst，也就是使用多个周期完成一个写请求的数据传输
1. 在 B Channel 上等待 Slave 回复写入完成的响应

为了完成一次读请求，Master 需要：

1. 通过 AR Channel 发送要读取的地址、读取的长度等信息给 Slave
1. 在 R Channel 上等待 Slave 回复读取的数据，支持 Burst，也就是使用多个周期完成一个读响应的数据传输
...
AXI 有支持缓存一致性协议的扩展：ACE，其内容在 [一致性协议](https://jia.je/kb/en/hardware/cache_coherence_protocol.html) 中介绍。
...
本文主要讨论前两种，TL-C 的内容在 [一致性协议](https://jia.je/kb/en/hardware/cache_coherence_protocol.html) 中介绍。
