...
Tesla 架构总览（来源：NVIDIA Tesla: A Unified Graphics and Computing Architecture Figure 1）
...
Tesla TPC 架构（来源：NVIDIA Tesla: A Unified Graphics and Computing Architecture Figure 2）
...
为了提升内存访问带宽，SM 会对同一个 warp 对 local 或者 global memory 的访问进行合并，把连续的访问合并成一个大的访问，从而提升性能。Tesla 架构还引入了原子指令，包括整数加法、整数最小最大值、逻辑运算、swap 和 Compare-And-Swap（CAS）。关于原子指令的部分，可以阅读 [原子指令](https://jia.je/kb/en/software/atomic_instructions.html) 的介绍。
...
CUDA 中，有 thread block 的概念，也就是 cooperative thread array（CTA），cooperative 指的是这些 thread 可以通过 shared memory 和 \_\_syncthreads 来协同完成一些计算。因为它们需要使用 shared memory，自然 CTA 会被归到某个 SM 下面，而不会跨越多个 SM 去执行。每个 SM 可以支持最多同时执行 8 个 CTA。
...
Fermi 架构总览（来源：Fermi GF100 GPU Architecture Figure 2）
...
Fermi 架构 SM（来源：Fermi GF100 GPU Architecture Figure 3）

Fermi 架构 SM 发射（来源：NVIDIA’s Fermi: The First Complete GPU Computing Architecture Figure 7）
...
Tesla 和 Fermi 架构对比（来源：Fermi Whitepaper Summary Table）
...
1. Hyper-Q：允许多个 CPU 核心同时向 GPU 提交任务，把硬件任务队列从 1 增加到了 32 个。每个 CUDA stream 会对应到一个硬件任务队列，因此增加硬件任务队列，可以减少 false dependency。
1. GPUDirect：支持 RDMA
...
Kepler 架构 SM（来源：NVIDIA’s Next Generation CUDA Compute Architecture: Kepler TM GK110/210）
...
Maxwell 架构 SM（来源：NVIDIA GeForce GTX 980 Whitepaper）
...
Pascal 架构 SM（来源：NVIDIA Tesla P100 Whitepaper）
...
Volta 架构 SM（来源：NVIDIA TESLA V100 GPU ARCHITECTURE Figure 5）
...
Turing 架构 SM（来源：NVIDIA TURING GPU ARCHITECTURE Figure 4）
...
Turing 架构 SM 微架构（来源：RTX ON – THE NVIDIA TURING GPU）

Turing 架构 MIO 微架构（来源：RTX ON – THE NVIDIA TURING GPU）
...
Ampere 架构 GA100 SM（来源：NVIDIA A100 Tensor Core GPU Architecture Figure 7）
...
Ampere 架构 GA102 SM（来源：NVIDIA NVIDIA AMPERE GA102 GPU ARCHITECTURE Figure 3）
...
Ada Lovelace 架构 SM（来源：NVIDIA ADA GPU ARCHITECTURE Figure 5）
...
Hopper 架构 SM（来源：NVIDIA H100 Tensor Core GPU Architecture Figure 7）
