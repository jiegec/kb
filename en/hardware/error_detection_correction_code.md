...
[ p_x(a) = \\sum\_{i=1}^k x_ia^{i-1} \\bmod q ]
...
1. IP（Internet Protocol）：2 字节的 Internet Checksum，保护一个 IP 头部的数据
1. PCIe TLP（Transaction Layer Packet）：4 字节的 CRC，保护一个 TLP 的数据
1. PCIe DLLP（Data Link Layer Packet）：2 或 4 字节的 CRC，保护一个 DLLP 的数据
1. CXL 68B FLIT：2 字节的 CRC，保护 64 字节的数据
1. CXL Standard 256B FLIT：8 字节的 CRC，6 字节的 FEC，保护 242 字节的数据
1. CXL Latency Optimized 256B FLIT：前半部分 6 字节的 CRC 保护 122 字节的数据，后半部分 6 字节的 CRC 保护 116 字节的数据；最后 6 字节的 FEC 保护完整的 FLIT
