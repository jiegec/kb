...
比较特别的是，Xilinx 的 `1G/2.5G PCS/PMA or SGMII` 这个 IP 有一个模式：`1000BASE-X`，这个模式下可以直接接到 SFP 接口上，此时PHY 的 PCS 和 PMA 层功能都在 FPGA 内实现，外置的光模块只需要实现 PMD 层，也就是光电转换。它还支持动态切换：如果是 SGMII 模式，外接了一个 PHY，就可以实现 1000BASE-T，通过网线传输；如果是 1000BASE-X 模式，直接接到 SFP 口上，就可以实现 1000BASE-X，用光纤传输。
