...
首先来看一个场景：Prover 知道一个固定的多项式 (p(x))，也知道它有哪些根，现在假设这些根都是整数，多项式的系数也都是整数。现在 Prover 想要向 Verifier 证明这个多项式有某几个根（不是全部），但又不想告诉 Verifier 这个多项式的所有系数是多少，怎么实现呢？

例如 (p(x)=x^3-3x^2+2x)，这个多项式的所有根是 (0, 1, 2)，想要向 Verifier 证明这个多项式有两个根 (0, 1)，怎么证明呢？

对于 Verifier 来说，公开信息是，有一个固定的未知系数的多项式 (p(x))，它有两个根 (0, 1)，那么这个多项式一定带有一个因式 (t(x)=(x-1)(x-2))，也就是说，存在整系数多项式 (h(x)) 满足：
...
1. Prover 计算 (p(x_0)) 和 (h(x_0))，发送给 Verifier
1. Verifier 计算，如果发现 (p(x_0)=(x_0-1)(x_0-2)h(x_0))，并且 (p(x_0)) 和 (h(x_0)) 都是整数，那么说明没有出错
...
1. Verifier 生成随机数 (s)
1. Verifier 计算 (s) 的幂次，再带入 (E(v))：(E(s^i) = g^{s^i})
1. Verifier 给 Prover 发送 (E(s^0), E(s^1), \\ldots, E(s^d))，其中 (d) 是多项式 (p(x)) 的阶数
1. Prover 已知 (p(s)=t(s)h(s))，计算 (E(p(s)), E(h(s)))，发送给 Verifier
1. Verifier 计算 (E(p(s))) 和 (E(h(s))^{t(s)})，判断两者是否相等：(E(h(s))^{t(s)}=(g^{h(s)})^{t(s)}=g^{h(s)t(s)}=g^{p(s)}=E(p(s)))
...
1. Verifier 生成一个随机数 (\\alpha)，计算 (a' = a^{\\alpha} \\pmod n)，把 (a) 和 (a') 发送给 Prover
1. Prover 要对 (a) 进行模幂操作，次数是 (c)，那么需要计算 (b=a^c \\pmod n) 以及 (b' = (a')^c \\pmod n)，把 (b) 和 (b') 发送给 Verifier
1. Verifier 验证：(b^{\\alpha} = b' \\pmod n)
...
1. Verifier 生成随机数 (s)
1. Verifier 计算 (s) 的幂次，再带入 (E(v))：(E(s^i) = g^{s^i})
1. Verifier 生成随机数 (\\alpha)，带入 (E(s^i)^{\\alpha} = g^{\\alpha{}s^{i}})
1. Verifier 给 Prover 发送 (E(s^0), E(s^1), \\ldots, E(s^d)) 和 (E(s^0)^{\\alpha}, E(s^1)^{\\alpha}, \\ldots, E(s^d)^{\\alpha})，其中 (d) 是多项式 (p(x)) 的阶数
1. Prover 已知 (p(s)=t(s)h(s))，计算 (E(p(s)), E(\\alpha{}p(s)), E(h(s)))，发送给 Verifier
1. Verifier 判断 (E(p(s))=E(h(s))^{t(s)}) 以及 (E(p(s))^{\\alpha}=E(\\alpha{}p(s)))

这样就证明了两点：

1. (E(p(s))=E(h(s))^{t(s)}) 证明了 (t(x)) 是 (p(x)) 的因式
1. (E(p(s))^{\\alpha}=E(\\alpha{}p(s))) 证明了 (p(x)) 是一个多项式
...
1. Verifier 计算 (s) 的幂次，再带入 (E(v))：(E(s^i) = g^{s^i})
1. Verifier 计算 (E(s^i)^{\\alpha} = g^{\\alpha{}s^{i}})
1. Verifier 给 Prover 发送 (E(s^0), E(s^1), \\ldots, E(s^d)) 和 (E(s^0)^{\\alpha}, E(s^1)^{\\alpha}, \\ldots, E(s^d)^{\\alpha})，其中 (d) 是多项式 (p(x)) 的阶数
1. Prover 已知 (p(s)=t(s)h(s))，计算 (E(p(s)), E(\\alpha{}p(s)), E(h(s)))
1. Prover 生成随机数 (\\delta)，计算 (E(\\delta{}p(s)), E(\\delta{}\\alpha{}p(s)), E(\\delta{}h(s)))，发送给 Verifier
1. Verifier 判断 (E(\\delta{}p(s))=E(\\delta{}h(s))^{t(s)}) 以及 (E(\\delta{}p(s))^{\\alpha}=E(\\delta{}\\alpha{}p(s)))
...
1. (E(\\delta{}p(s))^{\\alpha}=E(\\delta{}\\alpha{}p(s)))
...
1. (E(\\delta{}p(s))^{\\alpha}=E(\\delta{}\\alpha{}p(s))) 对应 (g^{\\delta{}p(s)} \\times g^{\\alpha} = g^{\\delta{}\\alpha{}p(s)} \\times g)
...
1. 初始化阶段
   1. 生成随机数 (s) 和 (\\alpha)
   1. 计算 (g^{s^i}, g^{\\alpha{}s^i}, g^{\\alpha}, g^{t(s)})
   1. 记其中的 (g^{s^i}, g^{\\alpha{}s^i}) 为 Proving Key，用于 Prover 生成 Proof
   1. 记其中的 (g^{\\alpha}, g^{t(s)}) 为 Verification Key，用于 Verifier 验证 Proof
1. 证明阶段
   1. 计算 (h(x) = p(x) / t(x))
   1. 用 (g^{s^i}) 计算 (g^{p(s)}) 和 (g^{h(s)})
   1. 用 (g^{\\alpha{}s^i}) 计算 (g^{\\alpha{}p(s)})
   1. 生成随机数 (\\delta)
   1. 计算 (g^{\\delta{}p(s)}, g^{\\delta{}h(s)}, g^{\\delta{}\\alpha{}p(s)})
1. 验证阶段
   1. 检查 (g^{\\delta{}\\alpha{}p(s)} \\times g = g^{\\delta{}p(s)} \\times g^{\\alpha})
   1. 检查 (g^{\\delta{}p(s)} \\times g = g^{t(s)} \\times g^{\\delta{}h(s)})

## 通用零知识证明

上面的零知识证明只适用于多项式的零点问题，那么对于更广泛的场景，例如前面提到的银行存款的问题，如何实现呢？

思路是把问题转化为前面的多项式零点问题，具体方法详见参考资料。
